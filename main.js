(() => {
  ...
  const PARTY_SLOTS = 6;
  const POLL_MS = 250;
  const STORAGE_KEY = "startab_dex_overlay_v2";
  ...
  const applySavedOverlay = box => {
    ...
    if (Number.isFinite(state.width))
      box.style.width = `${Math.max(280, Math.min(state.width, innerWidth - 8))}px`;
    if (Number.isFinite(state.height))
      box.style.height = `${Math.max(300, Math.min(state.height, innerHeight - 8))}px`;
  ...
  const createOverlay = () => {
    ...
    style.textContent = `
      #celarys-wild-stats-box {
        ...
        --sad-font:12px;font-family:VPPixel,monospace;font-size:14px;
        line-height:1;
        color:#fff;
        text-shadow:var(--tr-shadow-off,1px) var(--tr-shadow-off,1px) 0 #1F1F1F;
        overflow:visible;
        white-space:nowrap;
        background:linear-gradient(#e2454c,#bd252c);
        border:4px solid var(--darkred);
        border-radius:18px;
        box-shadow:0 7px 0 #54151a,0 14px 34px rgba(0,0,0,.42);
      }
      #celarys-wild-stats-box *{box-sizing:border-box;font-family:VPPixel,monospace}
      #celarys-wild-stats-box .cws-title b{font-size:22px;line-height:1;font-family:VPPixel,monospace;letter-spacing:.04em}
      #celarys-wild-stats-box .cws-title span{font-size:10px}
      #celarys-wild-stats-box .cws-name{font-size:22px;line-height:1}
      #celarys-wild-stats-box .cws-battle-kind{font-size:20px}
      #celarys-wild-stats-box .cws-hpline{font-size:20px}
      #celarys-wild-stats-box .cws-type{font-size:20px}
      #celarys-wild-stats-box .cws-label{font-size:22px}
      #celarys-wild-stats-box .cws-ability{font-size:22px}
      #celarys-wild-stats-box .cws-stats span{font-size:24px}
      #celarys-wild-stats-box .cws-stats b{font-size:24px}
      #celarys-wild-stats-box .cws-moves-title span{font-size:22px}
      #celarys-wild-stats-box .cws-moves-title small{font-size:22px}
      #celarys-wild-stats-box .cws-move-name{font-size:22px}
      #celarys-wild-stats-box .cws-num{font-size:22px}
      .cws-topbar{
        min-height: auto;
        display: grid;
        ...
        align-items: center;
        padding: 8px 12px;
        background: linear-gradient(rgba(255, 255, 255, .16), rgba(0, 0, 0, .05));
      }
      .cws-lens{
        width:36px;height:36px;border-radius:50%;border:4px solid #f1efe4;
        background:radial-gradient(circle at 34% 28%,#fff 0 8%,#a7eeff 10% 28%,#37aee0 31% 67%,#13648a 69%);
        box-shadow:0 0 0 2px #5b191d;
      }
      .cws-title{min-width:0;display:flex;flex-direction:column}
      .cws-title b{font-weight:900}
      .cws-title span{letter-spacing:.15em;color:#ffd9c8;margin-top:3px}
      .cws-leds{display:flex;gap:5px;align-self:start;margin-top:3px}
      .cws-leds i{width:9px;height:9px;border-radius:50%;background:#f4d44b;border:1px solid #65191e}
      .cws-leds i:nth-child(2){background:#67d86d} .cws-leds i:nth-child(3){background:#62a9ee}
      .cws-body,.cws-status{background:var(--screen);border:2px solid #111;box-shadow:inset 0 0 0 2px #333}
      .cws-status{min-height:170px;display:grid;place-items:center;padding:20px;color:var(--lime);font-weight:800;text-align:center}
      .cws-body{padding:8px}

      .cws-upper{display:grid;grid-template-columns:1fr;gap:8px}
      .cws-profile{min-width:0;display:grid;grid-template-columns:86px minmax(0,1fr);gap:9px;border-bottom:3px solid var(--line);padding-bottom:8px}
      .cws-sprite{min-height:86px;place-items:center;overflow:hidden;position:relative}
      .cws-sprite-sheet{width:74px;height:74px;image-rendering:pixelated;background-repeat:no-repeat}
      .cws-pokeball{width:52px;height:52px;border:5px solid #555;border-radius:50%;background:linear-gradient(#8f3034 0 44%,#555 45% 55%,#d9d9d3 56%)}
      .cws-profile-info{min-width:0;padding:4px 0}
      .cws-name-row{display:flex;gap:8px;align-items:flex-start;justify-content:space-between}
      .cws-name{font-weight:900;line-height:1;overflow-wrap:anywhere}
      .cws-battle-kind{flex:0 0 auto;padding:3px 6px;background:#f0c43e;color:#29230d;border-radius:3px;font-weight:900}
      .cws-hpline{margin:8px 0;color:#fff}
      .cws-types{display:flex;flex-wrap:wrap;gap:5px;margin-bottom:9px}
      .cws-type{padding:3px 7px;border:1px solid #d7d7d3;border-radius:3px;background:#363a38;font-weight:900}
      .cws-label{color:var(--muted);letter-spacing:.12em}
      .cws-ability{font-weight:900;color:#f4e927;margin-top:2px;overflow-wrap:anywhere}

      .cws-stats{display:grid;align-content:start;border-radius: 4px;padding: 4px 12px;overflow:hidden}
      .cws-stats>div{display:grid;grid-template-columns:1fr auto;gap:12px;padding:3px 7px;}
      .cws-stats span{font-weight:900}
      .cws-stats b{font-weight:900;font-variant-numeric:tabular-nums}
      .cws-stats .cws-bst{border-bottom:0;margin-top:3px;padding-top:5px}

      .cws-divider{height:3px;background:var(--line);margin:8px 0}
      .cws-moves-title,.cws-move{
        display:grid;grid-template-columns:minmax(0,1fr) 40px 46px 46px;gap:6px;align-items:center
      }
      .cws-moves-title{padding:0 8px 6px;font-weight:900;border-bottom:2px solid var(--line)}
      .cws-moves-title span{}
      .cws-moves-title small{text-align:center}
      .cws-move{min-height:42px;padding:6px 8px;}
      .cws-move:last-child{border-bottom:0}
      .cws-move-name{min-width:0;display:flex;align-items:center;gap:7px;font-weight:900;overflow:hidden}
      .cws-move-name span:last-child{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      .cws-cat{position:relative;width:16px;height:16px;flex:0 0 16px}
      .cws-cat.physical{
        background:#ffc928;
        clip-path:polygon(50% 0,61% 31%,85% 15%,69% 39%,100% 50%,69% 61%,85% 85%,61% 69%,50% 100%,39% 69%,15% 85%,31% 61%,0 50%,31% 39%,15% 15%,39% 31%)
      }
      .cws-cat.special{border:3px solid #1688ff;border-radius:50%}
      .cws-cat.special::after{content:"";position:absolute;inset:3px;border:2px solid #1688ff;border-radius:50%}
      .cws-cat.status::before{}
      .cws-num{text-align:center;font-weight:900;font-variant-numeric:tabular-nums}
      .cws-unknown{color:#888}

      .cws-bottom{display:flex;justify-content:space-between;gap:8px;flex-wrap:wrap;padding:0 13px 10px;color:#ffd9c8;font-size:8px;font-weight:900;letter-spacing:.08em}

      #celarys-wild-stats-box::-webkit-resizer{
        background:linear-gradient(135deg,transparent 0 35%,#fff1b8 36% 45%,transparent 46% 55%,#fff1b8 56% 65%,transparent 66%)
      }

      @container (max-width:430px){
        .cws-upper{grid-template-columns:1fr}
        .cws-profile{border-right:0;border-bottom:3px solid var(--line);padding-right:0;padding-bottom:8px;grid-template-columns:92px minmax(0,1fr)}
        .cws-sprite{min-height:92px}
        .cws-moves-title,.cws-move{grid-template-columns:minmax(0,1fr) 42px 48px 48px}
      }
      @container (max-width:330px){
        .cws-profile{grid-template-columns:1fr}
        .cws-sprite{display:none}
        .cws-moves-title,.cws-move{grid-template-columns:minmax(0,1fr) 38px 42px 42px;gap:3px}
        .cws-bottom span:last-child{display:none}
      }
    `;
  ...
  const fitDefaultLayout = () => {
    ...
    requestAnimationFrame(() => {
      box.style.width = `${Math.min(372, Math.max(320, innerWidth - 24))}px`;
      box.style.height = "auto";
      requestAnimationFrame(() => {
        const available = Math.max(320, innerHeight - 24);
  ...
  const tick = () => {
    try {
      ...
      if (!data.inBattle || !data.isWild || !data.enemy?.name) {
